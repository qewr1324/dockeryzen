...
import type { ProjectAnalysis, ProjectType, BuildTool, Framework, JdkVendor, OutputType, DatabaseConfig, DatabaseType, Dependency, ConfigFile } from '../../types/interfaces';
...
export class JavaProjectAnalyzer extends ProjectAnalyzer {
  ...
  public async analyze(): Promise<ProjectAnalysis> {
    ...
    return {
      projectType: buildTool === BuildTool.NONE ? ProjectType.UNKNOWN : 
                   (await this.isMultiModule() ? ProjectType.MULTI_MODULE : 
                   buildTool === BuildTool.MAVEN ? ProjectType.MAVEN : ProjectType.GRADLE),
      buildTool,
      framework,
      jdkVersion,
      jdkVendor,
  ...
  private getDefaultDbPort(dbType: DatabaseType): number {
    switch (dbType) {
      ...
      case DatabaseType.CASSANDRA: return 9042;
      case DatabaseType.ELASTICSEARCH: return 9200;
      case DatabaseType.NEO4J: return 7687;
      case DatabaseType.H2: return 9092;
      default: return 5432;
    }
  }
  ...
  protected async detectProfiles(): Promise<string[] | undefined> {
    ...
    const configFiles = await vscode.workspace.findFiles(
      new vscode.RelativePattern(this.workspaceFolder, '**/application-*.{properties,yml,yaml}'),
      '**/node_modules/**'
    );
  ...
  protected async extractEnvVariables(): Promise<Record<string, string>> {
    ...
    const configFiles = await vscode.workspace.findFiles(
      new vscode.RelativePattern(this.workspaceFolder, '**/application.properties'),
      '**/node_modules/**'
    );
  ...
  protected detectOutputType(framework: Framework, dependencies: Dependency[]): OutputType {
    ...
    if (framework === Framework.SPRING_MVC || framework === Framework.JAVA_EE || framework === Framework.JAKARTA_EE) {
      for (const dep of dependencies) {
        if (dep.artifactId.toLowerCase().includes('tomcat') || 
            dep.artifactId.toLowerCase().includes('jetty') ||
            dep.artifactId.toLowerCase().includes('undertow')) {
  ...
  protected detectJdkVendor(dependencies: Dependency[]): JdkVendor {
    ...
    switch (configuredVendor) {
      case 'eclipse-temurin': return JdkVendor.ECLIPSE_TEMURIN;
      case 'amazon-corretto': return JdkVendor.AMAZON_CORRETTO;
      case 'openjdk': return JdkVendor.OPENJDK;
      case 'oracle-jdk': return JdkVendor.ORACLE_JDK;
      case 'graalvm': return JdkVendor.GRAALVM;
      case 'liberica': return JdkVendor.LIBERICA;
      case 'redhat-openjdk': return JdkVendor.REDHAT_OPENJDK;
      default: return JdkVendor.ECLIPSE_TEMURIN;
    }
  }
