...
export class DockerfileGenerator {
	...
	private getDebugExpose(): string {
		...
		const lang = this.config.language;
		// Rust debug uses RUST_LOG, not a port
		if (lang === "rust") {
			return "";
		}
		// Go debug uses delve (dlv) on port 2345
		if (lang === "go") {
			return `\n# Debug port (delve)\nEXPOSE 2345`;
		}
		// C/C++ use gdbserver on 1234
		if (lang === "cpp" || lang === "c") {
	...
	private getHealthCheck(): string {
		...
		if (lang === "python") {
			const primaryCheck = `python -c "import urllib.request; urllib.request.urlopen('http://localhost:${port}${healthPath}')"`;
			const fallbackCheck = `python -c "import urllib.request; urllib.request.urlopen('http://localhost:${port}/')"`;
		...
		if (lang === "cpp" || lang === "c") {
			// C/C++ apps usually don't expose HTTP — use TCP port check
...
ENTRYPOINT ["sh", "-c", "java $JAVA_OPTS -jar app.jar"]`;
	...
	private generateJavaWarDockerfile(): string {
		...
		const startCommand = isJetty ? 'ENTRYPOINT ["/usr/local/jetty/bin/jetty.sh", "run"]' : 'ENTRYPOINT ["catalina.sh", "run"]';

		const workDirSection = isJetty ? "" : "WORKDIR /usr/local/tomcat\n";
		...
		const warSourcePath = this.config.buildTool === "gradle" ? "/app/build/libs" : "/app/target";
		const warCopyCmd = `RUN WAR_FILE=$(find ${warSourcePath} -name "*.war" -not -name "*-sources.war" -not -name "*-javadoc.war" | head -n 1) && \\
...
COPY --from=build ${warSourcePath} /tmp/wars/
${warCopyCmd}
${debugEnv}
...
RUN --mount=type=cache,target=/root/.m2 \\
    if [ -f mvnw ]; then chmod +x mvnw && ./mvnw package -DskipTests; else mvn package -DskipTests; fi
...
COPY . .
RUN npm run build -- --configuration production 2>/dev/null || npm run build
...
# Nuxt 3 uses .output/
COPY --from=build /app/.output ./.output 2>/dev/null || true
# Fallback for Nuxt 2
COPY --from=build /app/.nuxt ./.nuxt 2>/dev/null || true
COPY --from=build /app/node_modules ./node_modules
COPY --from=build /app/package.json ./
COPY --from=build /app/public ./public 2>/dev/null || true
COPY --from=build /app/nuxt.config.* ./ 2>/dev/null || true
...
CMD ["sh", "-c", "if [ -f .output/server/index.mjs ]; then exec node .output/server/index.mjs; elif [ -f .output/server/index.js ]; then exec node .output/server/index.js; elif [ -f .nuxt/dist/server/server.mjs ]; then exec node .nuxt/dist/server/server.mjs; else exec npm run start; fi"]`;
		} else {
			return `# syntax=docker/dockerfile:1.4
...
RUN if [ -f next.config.js ] || [ -f next.config.mjs ] || [ -f next.config.ts ]; then \\
        npm install sharp --no-save 2>/dev/null || true; \\
    fi
RUN npm run build
...
COPY --from=build /app/out ./out 2>/dev/null || true
COPY --from=build /app/node_modules ./node_modules
COPY --from=build /app/package.json ./
COPY --from=build /app/public ./public 2>/dev/null || true
...
${ociLabels}
EXPOSE ${port}${debugExpose}${healthCheck}
CMD ["sh", "-c", "if [ -f .next/standalone/server.js ]; then exec node .next/standalone/server.js; elif [ -f node_modules/.bin/next ]; then exec npx next start -p ${port} -H 0.0.0.0; else exec npm start; fi"]`;
		}
	}
	...
	private generateJSBackendDockerfile(): string {
		...
		const entryCheck = entryCandidates.map((p) => `elif [ -f ${p} ]; then exec ${usePm2 ? "pm2-runtime start" : "node"} ${p}${usePm2 ? " --name app" : ""};`).join(" \\\n            ");
...
${entryPointCmd}`;
	...
	private generatePythonDockerfile(): string {
		...
		if (framework === "fastapi") {
			extraPackages = extraPackages ? `${extraPackages} "uvicorn[standard]" gunicorn` : '"uvicorn[standard]" gunicorn';
		}
		...
		if (framework === "django") {
			...
			if (useGunicorn) {
				startCmd = `CMD ["sh", "-c", "if [ -f wsgi.py ]; then exec gunicorn --bind 0.0.0.0:${port} --workers 4 --timeout 120 wsgi:application; elif [ -f ${wsgiModule}/wsgi.py ]; then exec gunicorn --bind 0.0.0.0:${port} --workers 4 --timeout 120 ${wsgiModule}.wsgi:application; else exec python manage.py runserver 0.0.0.0:${port}; fi"]`;
			} else {
				startCmd = `CMD ["sh", "-c", "python manage.py migrate && python manage.py runserver 0.0.0.0:${port}"]`;
			}
		} else if (framework === "flask") {
			if (useGunicorn) {
				startCmd = `CMD ["sh", "-c", "if [ -f wsgi.py ]; then exec gunicorn --bind 0.0.0.0:${port} --workers 4 --timeout 120 wsgi:app; elif [ -f app.py ]; then exec gunicorn --bind 0.0.0.0:${port} --workers 4 --timeout 120 app:app; elif [ -f main.py ]; then exec gunicorn --bind 0.0.0.0:${port} --workers 4 --timeout 120 main:app; else exec flask run --host=0.0.0.0 --port=${port}; fi"]`;
			} else {
				startCmd = `CMD ["flask", "run", "--host=0.0.0.0", "--port=${port}"]`;
			}
		} else if (framework === "fastapi") {
			if (useGunicorn) {
				startCmd = `CMD ["sh", "-c", "WORKERS=\${WORKERS:-4}; if [ -f main.py ]; then exec gunicorn main:app --bind 0.0.0.0:${port} --workers $WORKERS --worker-class uvicorn.workers.UvicornWorker --timeout 120; elif [ -f app.py ]; then exec gunicorn app:app --bind 0.0.0.0:${port} --workers $WORKERS --worker-class uvicorn.workers.UvicornWorker --timeout 120; else exec uvicorn main:app --host 0.0.0.0 --port ${port} --workers $WORKERS; fi"]`;
			} else {
				startCmd = `CMD ["sh", "-c", "WORKERS=\${WORKERS:-4}; if [ -f main.py ]; then exec uvicorn main:app --host 0.0.0.0 --port ${port} --workers $WORKERS --loop uvloop --http httptools; elif [ -f app.py ]; then exec uvicorn app:app --host 0.0.0.0 --port ${port} --workers $WORKERS --loop uvloop --http httptools; else exec uvicorn main:app --host 0.0.0.0 --port ${port} --workers $WORKERS; fi"]`;
			}
		}
...
${startCmd}`;
	...
	private generateGoDockerfile(): string {
		...
		// Runtime deps
		const runtimePackages = this.config.useAlpine ? "RUN apk --no-cache add ca-certificates tzdata curl wget" : "RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates tzdata curl wget && rm -rf /var/lib/apt/lists/*";

		// LD flags only if user has main.version (safe fallback)
		const ldflags = "-s -w -X main.version=1.0.0";

		// Proper mod download with go.sum handling
...
CMD ["./main"]`;
	...
	private generateRustDockerfile(): string {
		...
		const runtimePackages = this.config.useAlpine ? "RUN apk --no-cache add ca-certificates libgcc libssl3" : "RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates libssl3 libgcc-s1 && rm -rf /var/lib/apt/lists/*";
...
RUN mkdir -p src && \\
    if [ ! -f src/main.rs ]; then echo "fn main() {}" > src/main.rs; fi && \\
    if [ ! -f src/lib.rs ]; then echo "" > src/lib.rs; fi
...
# Copy final binary to a stable location
RUN BIN=$(find /app/target/release -maxdepth 1 -type f -executable -not -name "*.d" -not -name "*.rlib" -not -name "*.so" | head -n 1) && \\
...
USER appuser
${ociLabels}
EXPOSE ${this.config.port}${this.getDebugExpose()}${this.getHealthCheck()}
CMD ["./app"]`;
	}
...
`
		...
		const finalCmd = hasQueueWorker ? `CMD ["/usr/bin/supervisord", "-c", "/etc/supervisor/conf.d/supervisord.conf"]` : `CMD ["php-fpm"]`;
...
${finalCmd}`;
	...
	private generateRailsDockerfile(): string {
		...
		const runtimeDeps = this.config.useAlpine ? "RUN apk add --no-cache libpq mysql-client nodejs npm tzdata" : "RUN apt-get update && apt-get install -y --no-install-recommends libpq5 default-libmysqlclient-dev nodejs npm tzdata && rm -rf /var/lib/apt/lists/*";
    ...
    if [ -f Gemfile.lock ]; then \\
        ...
        bundle install --jobs 4 --retry 3 && bundle lock; \\
    fi`;
...
CMD ["sh", "-c", "if [ -f bin/rails ]; then bundle exec rails db:prepare 2>/dev/null || bundle exec rails db:migrate 2>/dev/null || true; fi; exec bundle exec puma -b tcp://0.0.0.0:${port} -e production"]`;
	...
	private generateCppDockerfile(): string {
		...
		// Build logic: try CMake, Makefile, or direct compile
    ...
    BIN=$(find /app /app/build /app/bin /app/src -maxdepth 3 -type f -executable \\
        -not -name "*.so" -not -name "*.cmake" -not -name "Makefile" \\
...
CMD ["./app"]`;
	...
	private generateCDockerfile(): string {
		...
		// musl is already part of alpine base — only libgcc needed
