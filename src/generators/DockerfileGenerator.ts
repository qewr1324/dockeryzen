...
export class DockerfileGenerator {
	...
	private getOciLabels(): string {
		const labels = [
			...
			`org.opencontainers.image.description="Generated by Dockeryzen"`,
			`org.opencontainers.image.version="1.0.0"`,
			`org.opencontainers.image.created="${new Date().toISOString()}"`,
			`org.opencontainers.image.source="https://github.com/yourusername/${this.config.projectName}"`,
		];
		return `# OCI Labels\nLABEL ${labels.join(" \\\n      ")}`;
	}
...
RUN --mount=type=cache,target=/root/.gradle/caches \\
    if [ -f gradlew ]; then ./gradlew build -x test --no-daemon; else gradle build -x test --no-daemon; fi`;
			...
			let image: string;
			if (version === "8") {
				image = useAlpine ? "maven:3.9-eclipse-temurin-8-alpine" : "maven:3.9-eclipse-temurin-8";
			} else {
	...
	private getServerImage(server: string): string {
		...
		if (this.langConfig?.types) {
			...
			if (warConfig?.servers) {
				...
				if (serverConfig) {
					if (useAlpine && serverConfig.alpineImages?.[version]) return serverConfig.alpineImages[version];
					if (serverConfig.images?.[version]) return serverConfig.images[version];
				}
			}
		}
...
CMD ["npm", "start"]`;
	...
	private generateJSBackendDockerfile(): string {
		...
		const pm2Cmd = this.config.enablePm2 ? 'CMD ["pm2-runtime", "start", "dist/index.js", "--name", "app", "-i", "max"]' : 'CMD ["node", "dist/index.js"]';
...
ENV PATH="/opt/venv/bin:$PATH"`
		...
		if (framework === "django") {
			...
			startCmd = `CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "${this.config.port}", "--workers", "4"]`;
		}
...
WORKDIR /app
${this.config.useAlpine && cgoEnabled ? "RUN apk add --no-cache gcc musl-dev" : ""}
...
ENTRYPOINT ["dotnet", "${safeProjectName}.dll"]`;
	...
	private generateLaravelDockerfile(): string {
		...
		const installCmd = this.config.useAlpine
			? `RUN apk add --no-cache git curl libpng-dev oniguruma-dev libxml2-dev zip unzip supervisor nginx`
			: `RUN apt-get update && apt-get install -y --no-install-recommends git curl libpng-dev libonig-dev libxml2-dev zip unzip supervisor nginx && rm -rf /var/lib/apt/lists/*`;
