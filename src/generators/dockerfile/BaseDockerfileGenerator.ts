...
export abstract class BaseDockerfileGenerator {
	...
	protected getHealthCheckInstall(): string {
		...
		// ✅ FIX: برای JS، اگر wget در image موجود نباشه، نصبش می‌کنیم.
		if (lang === "js-frontend" || lang === "js-backend") {
			if (useAlpine) {
				// در alpine، wget به صورت built-in هست (busybox) ولی برای اطمینان:
				return `\n# Install health check tools\nRUN apk add --no-cache ca-certificates curl\n`;
			}
			return `\n# Install health check tools\nRUN apt-get update && apt-get install -y --no-install-recommends ca-certificates curl wget && rm -rf /var/lib/apt/lists/*\n`;
		}

		// ✅ FIX: برای Laravel، چون health check با php -r انجام می‌شه، نیازی به curl نیست.
		if (lang === "laravel") {
			return ""; // هیچ ابزار اضافه‌ای نیاز نیست
		}
	...
	protected getOciLabels(): string {
		// ✅ FIX: استفاده از timestamp قابل تنظیم برای reproducibility.
		// اگر config.timeStamp وجود داشته باشه ازش استفاده می‌کنیم، وگرنه از یک مقدار ثابت.
	...
	protected getHeader(): string {
		...
		// ── عنوان: Framework یا Server یا Language ──
